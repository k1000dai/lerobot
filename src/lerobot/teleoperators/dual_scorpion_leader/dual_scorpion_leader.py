...
class DualScorpionLeader(Teleoperator):
    ...
    def action_features(self) -> dict[str, type]:
        ...
        features.update({f"{motor}.pos": float for motor in self.right_bus.motors})
        #  Left arm motors / 左腕のモーター
        features.update({f"{motor}.pos": float for motor in self.left_bus.motors})
        return features
    ...
    def calibrate(self) -> None:
        ...
        for (
            ...
            self.right_bus.motors.items()
        ):  # motor = "shoulder_pan", "shoulder_lift", ... | m = Motor instance
            self.calibration[f"{motor}"] = MotorCalibration(
                id=m.id,
                drive_mode=0,
        ...
        right_calibration = {
            motor: calib for motor, calib in self.calibration.items() if motor.startswith("arm_right")
        }
        # Extract calibration data for the left arm / 左腕用のキャリブレーションデータを抽出
        left_calibration = {
            motor: calibrate for motor, calibrate in self.calibration.items() if motor.startswith("arm_left")
        }
